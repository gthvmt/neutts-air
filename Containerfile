# Dockerfile for NeuTTS Air (uses espeak-ng)
# Build args:
#   PYTHON_VERSION (default 3.11)
#   INSTALL_LLAMA (default "false") -- set to "true" to pip-install llama-cpp-python
#   INSTALL_ONNX  (default "false") -- set to "true" to pip-install onnxruntime
# Usage examples:
#  docker build -t neutts-air:latest .
#  docker build --build-arg INSTALL_LLAMA=true --build-arg INSTALL_ONNX=true -t neutts-air:latest .
...
# Build-time args to optionally install extras
...
# Optionally install llama-cpp-python (for GGUF) and onnxruntime
# These are optional and can be big; use build args to include them only when needed.
RUN if [ "${INSTALL_LLAMA}" = "true" ]; then \
      pip install "llama-cpp-python"; \
    else \
      echo "Skipping llama-cpp-python"; \
    fi
